...
library LibGaugeLogic {
    ...
    */
            ...
            for(uint i = 0; i < gd.distributions.length; i++) {
                ...
                if(lpDist.impl.target != address(0)) {
                    ...
                    if(lpDist.impl.encodeType == bytes1(0x00)) {
                        (success, returnData) = lpDist.impl.target.staticcall(abi.encodeWithSelector(
                    ...
                    if(lpDist.delta > 0) {
                        uint64 newOptimalPercentDepositedBdv = calculateOptimalPercentDepositedBdv(lpDist.token, lpDist.delta);
    ...
    function calculateOptimalPercentDepositedBdv(
        ...
        AppStorage storage s = LibAppStorage.diamondStorage();
        uint64 currentOptimalPercentDepositedBdv = s.sys.silo.assetSettings[token].optimalPercentDepositedBdv;
        if(delta > 0) {
            // if the new optimal percent deposited bdv is greater than the maximum, set it to the maximum.
            if(currentOptimalPercentDepositedBdv + uint64(delta) > uint64(LibSeedGauge.OPTIMAL_DEPOSITED_BDV_PERCENT)) {
