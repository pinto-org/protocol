 ...
 */
contract SowWithAssetsBlueprint is BlueprintBase {
    /**
     ...
     * @param assets Indices of source tokens to withdraw from
    ...
    struct Asset {
        address wellToken; // well to buy from (implies token to use)
        uint256 amount;
    }
    ...
    function sowBlueprintv0(
        ...
        // purchase beans from sources
        purchaseBeansFromAssets(params.sowParams.assets, vars.totalBeansNeeded, slippageRatio);
    ...
    ) internal view returns (uint256 availableSoil, uint32 currentSeason) {
    ...
    {
        (availableSoil, currentSeason) = getAndValidateBeanstalkState(params.sowParams);
    ...
    // function purchases pinto from well. balances are internal balances.
    function purchaseBeansFromAssets(
        Asset[] memory assets,
        uint256 totalBeansNeeded,
        uint256 slippageRatio
    ) internal {
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            // if nonzero, check slippage. Users can opt out of slippage checks for gas efficency.
            // networks with private mempools may be less suspecitble to MEV attacks.
            // however, tractor orders that are public may be frontran.
            isValidSlippage(assets[i].wellToken, assets[i].token, slippageRatio);
            (uint256 tokenAmount, uint256 beansRemaining) = calculateAmount(
                assets[i],
                totalBeansNeeded
            );
            uint256 beansGained = transferAndShift(
                assets[i].wellToken,
                publisher,
                assets[i].token,
                tokenAmount
            ...
            beanstalk.sendTokenToInternalBalance(IERC20(beanToken), publisher, beansGained);
        }
    }

    // returns tokenAmount to sow, beansRemaining (if applicable
    function calculateAmount(
        Asset memory asset,
        uint256 beanAmount
    ) internal returns (uint256, uint256) {
        address token = getTokenFromWell(asset.wellToken);
        uint256 tokenAmount = IWell(asset.wellToken).getSwapIn(
            IERC20(token),
            IERC20(beanToken),
            beanAmount
        );
        // if the amount
        uint256 beansUsed;
        if (tokenAmount >= asset.amount) {
            tokenAmount = asset.amount;
            beansUsed = IWell(asset.wellToken).getSwapOut(
                IERC20(token),
                IERC20(beanToken),
                tokenAmount
            );
        }
        return (tokenAmount, beanAmount - tokenAmount);
    }

    // * transfering and shifting removes an approval step.
    function transferAndShift(
        address well,
        address publisher,
        address token,
        uint256 tokenAmount
    ) internal {
        IERC20(token).transferFrom(publisher, well, tokenAmount);
        IWell(well).shift(token, 0, address(this));
    }

    function getTokenFromWell(address wellToken) internal returns (address) {
        address[] memory tokens = IWell(wellToken).tokens();
        return tokens[0] == beanToken ? tokens[1] : tokens[0];
    }
}
